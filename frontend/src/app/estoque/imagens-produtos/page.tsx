...
- o manifesto atual do S3 confirma a existência do arquivo antes de exibir uma pendência antiga no grid.
...
export default function FinanceiroEstoqueImagensProdutosPage() {
  ...
  const [s3ManifestProductIds, setS3ManifestProductIds] = useState<Set<string>>(() => new Set());
  ...
  useEffect(() => {
    ...
    const loadImageLocations = async () => {
      ...
      if (!runtimeContext.sourceSystem || !runtimeContext.sourceTenantId) {
        setS3ImagesFolder('');
        setS3AccessOrigin('');
        setS3ManifestProductIds(new Set());
        return;
      }
      ...
      const manifestResult = await getJson<ProductImageManifest>(
      ...
      if (active) {
        ...
        setS3ManifestProductIds(new Set((manifestResult?.files || []).map((file) => file.productId)));
      }
    };
  ...
  const filteredProducts = useMemo(() => {
    ...
    return s3SyncFilter === 'PENDING'
      ? searchedProducts.filter((product) => isProductS3Pending(product))
      : searchedProducts;
  }, [products, s3SyncFilter, search, s3ManifestProductIds]);
  ...
  function isProductS3Pending(product: ProductItem) {
    return product.imageS3SyncStatus === 'PENDING'
      && !s3ManifestProductIds.has(product.id);
  }
  ...
  async function replaceProductImage(url: string, product = replacementProduct) {
    ...
    try {
      ...
      try {
        ...
        setS3ManifestProductIds((current) => new Set(current).add(product.id));
      } catch (syncError) {
        ...
        setS3ManifestProductIds((current) => {
    ...
    <div className="flex min-h-[calc(100vh-11rem)] flex-col gap-4 p-4 sm:p-6">
      <section className="shrink-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] font-bold text-amber-800" title={s3ImagesFolder || 'Pasta não configurada no S3'}>
              <span><span className="font-black uppercase tracking-[0.12em]">Pasta S3:</span> {s3ImagesFolder || 'NÃO INFORMADA'}</span>
              <span><span className="font-black uppercase tracking-[0.12em]">Origem do acesso:</span> {s3AccessOrigin || 'NÃO INFORMADA'}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => { setS3SyncFilter((current) => current === 'PENDING' ? 'ALL' : 'PENDING'); setCurrentPage(1); }} className={`rounded-xl border px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] transition ${s3SyncFilter === 'PENDING' ? 'border-amber-500 bg-amber-500 text-white' : 'border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100'}`}>
              Pendentes S3: {products.filter((product) => isProductS3Pending(product)).length}
            </button>
            <button
              type="button"
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table
            className="grid h-auto min-w-[1010px] w-full border-collapse text-left"
            style={{ height: `${Math.max(1, visibleProducts.length) * 64 + 40}px` }}
          >
            <thead className="sticky top-0 z-10 block w-full bg-slate-100 text-[10px] font-black uppercase tracking-[0.14em] text-slate-600">
              <tr className="grid w-full grid-cols-[80px_minmax(0,1fr)_160px_190px_160px_160px]">
                <th className="min-w-0 px-4 py-3">Imagem</th>
                <th className="min-w-0 px-4 py-3">Produto</th>
                <th className="min-w-0 px-4 py-3">Código interno</th>
                <th className="min-w-0 px-4 py-3">Código de barras</th>
                <th className="min-w-0 px-4 py-3">Situação</th>
                <th className="min-w-0 px-1 py-3 text-center">Ação</th>
              </tr>
            </thead>
            <tbody className="block w-full">
              {isLoading ? (
                ...
                visibleProducts.map((product, index) => {
                  ...
                  return (
                    <tr key={product.id} className={`${index % 2 ? 'bg-slate-100/70' : 'bg-white'} grid w-full grid-cols-[80px_minmax(0,1fr)_160px_190px_160px_160px] items-center !h-16`}>
