...
type DdaStatusFilter = 'OPEN' | 'CLOSED' | 'CANCELED';
...
function getDdaStatusTone(value?: string | null) {
  ...
  if (normalized === 'CANCELED') return 'border-rose-200 bg-rose-50 text-rose-700';
  if (normalized.includes('VENC')) return 'border-rose-200 bg-rose-50 text-rose-700';
  if (normalized === 'OPEN' || normalized.includes('ABERTO') || normalized.includes('PEND')) return 'border-amber-200 bg-amber-50 text-amber-700';
...
export default function FinanceiroOpenDdasPage() {
  ...
  const loadOpenDdas = useCallback(async () => {
    ...
    try {
      ...
      setDdaItems(response.items || []); setPulledAt(response.pulledAt || null); setStatusMessage(response.message || `${response.ddaCount || response.items?.length || 0} DDA(s) em aberto encontrado(s).`);
    ...
    <div className={runtimeContext.embedded ? 'flex h-screen min-h-0 flex-col overflow-hidden' : FINANCE_GRID_PAGE_LAYOUT.shell}>
      ...
      <section className={`${cardClass} flex min-h-0 flex-1 flex-col overflow-hidden`}>
        <div className="shrink-0 border-b border-slate-100 bg-slate-50 px-5 py-4">
          <div className="grid gap-3 xl:grid-cols-[minmax(15rem,1fr)_minmax(28rem,2fr)_auto] xl:items-end"><label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Banco</span>{lockedBankId ? <div className="min-h-[46px] rounded-xl border border-slate-300 bg-slate-100 px-4 py-3 text-sm font-black uppercase text-slate-700">{selectedBank ? buildBankLabel(selectedBank) : 'BANCO SELECIONADO'}</div> : <select value={selectedBankId} onChange={(event) => setSelectedBankId(event.target.value)} className={FINANCE_GRID_PAGE_LAYOUT.input}><option value="">SELECIONE</option>{banks.map((item) => <option key={item.id} value={item.id}>{buildBankLabel(item)}</option>)}</select>}</label><label className="block"><span className="sr-only">Pesquisar no grid</span><input value={generalSearch} onChange={(event) => setGeneralSearch(event.target.value.toUpperCase())} className={`${FINANCE_GRID_PAGE_LAYOUT.input} w-full`} placeholder="BENEFICIÁRIO, DOCUMENTO, PAGADOR..." /></label><button type="button" onClick={() => void loadOpenDdas()} disabled={isConsulting || !selectedBankId} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-xs font-black uppercase tracking-[0.18em] text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60">{isConsulting ? 'Consultando...' : 'Atualizar'}</button></div>
          <div className="mt-3 flex flex-wrap items-center gap-2">{([{ value: 'OPEN', label: 'Abertos', tone: 'border-amber-300 bg-amber-50 text-amber-700' }, { value: 'CLOSED', label: 'Fechados', tone: 'border-emerald-300 bg-emerald-50 text-emerald-700' }, { value: 'CANCELED', label: 'Cancelados', tone: 'border-rose-300 bg-rose-50 text-rose-700' }] as const).map((option) => <button key={option.value} type="button" onClick={() => setStatusFilter(option.value)} aria-pressed={statusFilter === option.value} className={`rounded-full border px-4 py-2 text-[10px] font-black uppercase tracking-[0.14em] transition ${statusFilter === option.value ? `${option.tone} ring-2 ring-slate-300 ring-offset-1` : 'border-slate-200 bg-white text-slate-500'}`}>{option.label} ({statusCounts[option.value]})</button>)}</div>
        </div>
