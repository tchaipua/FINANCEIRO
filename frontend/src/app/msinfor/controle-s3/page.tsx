...
import { financeApiFetch, requestJson } from '@/app/lib/api';
...
type Configuration = { configured: boolean; active?: boolean; endpoint?: string; region?: string; bucket?: string; basePrefix?: string; accessKeyConfigured?: boolean; secretKeyConfigured?: boolean; forcePathStyle?: boolean; capacityGb?: number | null; imagesFolder?: string; description?: string; sourceScope?: string };
...
type UploadProgress = { completed: number; total: number; skipped: number; selected: number; phase: 'CHECKING' | 'UPLOADING' };
...
function configurationOriginLabel(sourceScope?: string) { return sourceScope === 'BRANCH' ? 'FILIAL' : sourceScope === 'SOFTHOUSE' ? 'SOFTHOUSE' : 'EMPRESA'; }
...
export default function ControleS3Page() {
  ...
  const contextPayload = useMemo(() => ({ sourceSystem: runtimeContext.sourceSystem, sourceTenantId: runtimeContext.sourceTenantId, sourceBranchCode: runtimeContext.sourceBranchCode, requestedBy: runtimeContext.cashierDisplayName || runtimeContext.cashierUserId || 'ADMIN_FINANCEIRO' }), [runtimeContext]);
  const apiQuery = useMemo(() => buildFinanceApiQueryString(runtimeContext, { sourceBranchCode: runtimeContext.sourceBranchCode }), [runtimeContext]);

  const loadConfiguration = useCallback(async () => {
    try {
      const loaded = await requestJson<Configuration>(`/s3-control/effective-configuration${apiQuery}`);
      setMessage(null);
  ...
  useEffect(() => { if (runtimeContext.embedded && window.parent !== window) postMessageToTrustedParent({ type: 'MSINFOR_SCREEN_CONTEXT', screenId: EMBEDDED_SCREEN_ID }); }, [runtimeContext.embedded]);
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const all = isSearchMode
      ? files
      : prefix === ''
      ? [...folders, ...files, { id: 'F-ROOT', type: 'ROOT' as const, name: 'RAIZ', key: '' }]
      : [...folders, ...files];
    return all;
  ...
  const uploadFiles = async (files?: FileList | File[]) => {
    ...
    setUploadProgress({ completed: 0, total: selectedFiles.length, skipped: 0, selected: selectedFiles.length, phase: 'CHECKING' });
    let completed = 0;
    try {
      const namesQuery = new URLSearchParams(apiQuery.replace(/^\?/, ''));
      if (uploadTargetPrefix) namesQuery.set('prefix', uploadTargetPrefix);
      const existing = await requestJson<{ names: string[] }>(`/s3-control/objects/names?${namesQuery.toString()}`);
      const existingNames = new Set((existing.names || []).map((name) => name.toLocaleLowerCase('pt-BR')));
      const filesToUpload = selectedFiles.filter((file) => !existingNames.has(file.name.toLocaleLowerCase('pt-BR')));
      const skipped = selectedFiles.length - filesToUpload.length;
      setUploadProgress({ completed: 0, total: filesToUpload.length, skipped, selected: selectedFiles.length, phase: 'UPLOADING' });
      if (!filesToUpload.length) {
        setMessage(`${skipped} arquivo(s) já estavam no S3; nenhum reenvio foi necessário.`);
      ...
      const uploadFile = async (file: File) => {
        ...
        const response = await financeApiFetch('/s3-control/upload', { method: 'POST', body: formData });
        const payload = await response.json().catch(() => null);
        if (!response.ok) throw new Error(payload?.message || 'Não foi possível enviar um dos arquivos ao S3.');
        completed += 1;
        setUploadProgress((current) => current ? { ...current, completed } : current);
      };
      for (let start = 0; start < filesToUpload.length; start += UPLOAD_CONCURRENCY) {
      ...
      await loadListing(prefix, undefined, showRootFiles);
    } catch (error: any) { const reason = error?.message === 'Failed to fetch' ? 'A conexão com o servidor foi interrompida durante o envio.' : error?.message || 'Não foi possível enviar os arquivos ao S3.'; setMessage(`${reason} ${completed ? `${completed} arquivo(s) foram enviados antes da interrupção.` : 'Nenhum arquivo foi confirmado como enviado.'}`); } finally { setIsUploading(false); if (fileInputRef.current) fileInputRef.current.value = ''; window.setTimeout(() => setUploadProgress(null), 3000); }
  };
  const confirmUpload = () => {
  ...
  return <div className="flex h-[calc(100vh-1rem)] min-h-0 flex-col gap-5">
    ...
    <AuditedPopupShell isOpen={Boolean(uploadTarget)} screenId={UPLOAD_POPUP_ID} title="Enviar arquivos ao S3" eyebrow="Confirmação de destino" description={`Deseja selecionar um ou mais arquivos para enviar para ${uploadTarget?.key.includes('/') ? 'a subpasta' : 'a pasta'}: ${uploadTarget?.name || ''}?`} brandingName={runtimeContext.companyName} logoUrl={runtimeContext.logoUrl} onClose={() => !isUploading && setUploadTarget(null)} originText={ORIGIN} auditText={`Seleção de arquivos para: ${uploadTarget?.key || 'NÃO IDENTIFICADO'}.`} sqlText="PUT OBJECT <pasta-ou-subpasta>/<arquivo>; INSERT s3_audit_events." footerActions={<><button type="button" onClick={() => setUploadTarget(null)} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600">Cancelar</button><button type="button" disabled={isUploading} onClick={confirmUpload} className="rounded-xl bg-violet-700 px-4 py-2 text-sm font-black text-white disabled:opacity-60">Selecionar arquivos</button></>}><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm font-bold text-violet-900"><div>Destino: <span className="break-all font-black">{uploadTarget?.key}</span></div><div className="mt-3 text-base font-black text-red-600">Limite por envio: máximo de <span className="text-xl">{MAX_UPLOAD_FILES}</span> arquivos.</div></div></AuditedPopupShell>
    <AuditedPopupShell isOpen={pendingUploadFiles.length > 0} screenId={UPLOAD_POPUP_ID} title="Confirmar envio de arquivos" eyebrow="Arquivos selecionados" description={`Foram selecionados ${pendingUploadFiles.length} arquivo(s) para envio ao S3. O limite é de ${MAX_UPLOAD_FILES} por envio.`} brandingName={runtimeContext.companyName} logoUrl={runtimeContext.logoUrl} onClose={() => { if (!isUploading) { setPendingUploadFiles([]); if (fileInputRef.current) fileInputRef.current.value = ''; } }} originText={ORIGIN} auditText={`Confirmação de envio de ${pendingUploadFiles.length} arquivo(s) para: ${uploadTargetPrefix || 'RAIZ'}.`} sqlText="PUT OBJECT <pasta-ou-subpasta>/<arquivo>; INSERT s3_audit_events." footerActions={<><button type="button" onClick={() => { setPendingUploadFiles([]); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600">Cancelar</button><button type="button" disabled={isUploading} onClick={() => { const files = pendingUploadFiles; setPendingUploadFiles([]); void uploadFiles(files); }} className="rounded-xl bg-violet-700 px-4 py-2 text-sm font-black text-white disabled:opacity-60">Enviar {pendingUploadFiles.length} arquivo(s)</button></>}><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-center text-sm font-bold text-violet-900"><div className="text-3xl font-black">{pendingUploadFiles.length}</div><div className="mt-1 uppercase tracking-[0.12em]">arquivo(s) serão enviados</div><div className="mt-3 text-xs font-semibold text-violet-700">Limite por envio: {MAX_UPLOAD_FILES} arquivos.</div><div className="mt-3 text-xs font-semibold text-violet-700">O envio será feito em fila: o próximo arquivo começa somente após a conclusão do anterior.</div><div className="mt-3 text-xs font-semibold text-violet-700">Destino: <span className="break-all font-black">{uploadTargetPrefix || 'RAIZ'}</span></div></div></AuditedPopupShell>
